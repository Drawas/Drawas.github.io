...
for (var i = 2; i < 3 * (screen.height / 100); i++) {
...
function updatePositions() {
 ...
 var phases = [];
 for (var i = 0; i < 5; i++) {
  phases[i] = Math.sin(topScroll + i);
 }
 for (var i = 0; i < items.length; i++) {
  var leftPx = 100 * phases[i % 5];
  items[i].style.transform = 'translateX(' + leftPx + 'px)';
  //items[i].style.left = leftPx + 'px';
 }
...
document.addEventListener('DOMContentLoaded', function() {
 var cols = 8;
 var s =  256;
 for (var i = 0; i <  cols*(screen.height/s); i++) {
